...
function getOrbitPoint(progress: number): Point {
	const startAngle = degreesToRadians(MAIN_ORBIT.startDegrees);

	const endAngle = degreesToRadians(MAIN_ORBIT.endDegrees);

	const angle = lerp(startAngle, endAngle, progress);

	return {
		x: MAIN_ORBIT.cx + MAIN_ORBIT.rx * Math.cos(angle),
		y: MAIN_ORBIT.cy + MAIN_ORBIT.ry * Math.sin(angle),
	};
}
...
export default function OrbitCarousel({
	...
	const [gridDots, setGridDots] = useState<Record<string, Point[]>>({});

	const gridPathRefs = useRef<Record<string, SVGPathElement | null>>({});
	...
	return (
		<section
			...
			className={[
				...
				"h-[540px] sm:aspect-[12/7] sm:h-auto",
				...
				<g>
					{gridLines.map((line) => (
						<path
							key={line.id}
							ref={(element) => {
								gridPathRefs.current[line.id] = element;
							}}
							d={line.d}
							stroke="white"
				...
				<g>
					{positionedPeople.map((profile) => (
						<g
							...
							onClick={() => {
								if (profile.isVisible) {
									setActiveIndex(profile.personIndex);
								}
							}}
							style={{
								transform: `translate(${profile.point.x.toFixed(4)}px, ${profile.point.y.toFixed(4)}px) scale(${profile.scale})`,
								transformOrigin: "0 0",
						...
						>
							...
							<circle cx="0" cy="0" r="85" fill="#dce8ff" />
...
function SocialLinks({ socials }: { socials?: OrbitPerson["socials"] }) {
	const socialItems = [
		socials?.linkedin
			? {
					key: "linkedin",
