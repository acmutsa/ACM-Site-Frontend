				...
				<div className="flex items-center justify-end gap-2 md:hidden">
					<Sheet>
						...
						<SheetContent className="h-[100dvh] w-[90%] max-w-sm overflow-y-auto">
							<div className="mt-8 flex flex-col items-start gap-y-4 pr-2">
								{" "}
...
function ResourcesDropdown({
	...
	const resources = [
		...
		{
			name: "Elections",
			href: "https://wiki.acmutsa.org/ACM-2026-ACM-Officer-Election-335c7f3b3742802d93abff81c5a1002f",
			isExternal: true,
		},
	];
		...
		<div className="group relative">
			...
			<div
				className={`
					...
					top-full z-50 mt-3
					w-40 translate-y-1 rounded-xl
					p-2 opacity-0 shadow-lg transition-all duration-150 group-focus-within:visible
					group-focus-within:translate-y-0 group-focus-within:opacity-100 group-hover:visible
			...
			>
				{suborgs.map((s) => (
					<Link
						...
						className={`block rounded-lg px-3 py-2 text-sm ${itemHoverClass}`}
						style={
							isCustomColor ? { color: linkStyles } : undefined
						}
					>
						{s.name}
					</Link>
